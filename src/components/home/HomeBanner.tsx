...
export const defaultBannerSlides: BannerSlide[] = [
  { id: "b1", title: "上架 Agent Hunt 即可免费瓜分万亿曝光", cta: "立即体验", link: "#", active: true, gradient: "from-blue-600/90 via-indigo-600/80 to-violet-700/90" },
  { id: "b2", title: "CSDN 创作者计划 — 加入 AI 流量扶持计划", cta: "了解详情", link: "#", active: true, gradient: "from-emerald-600/90 via-teal-600/80 to-cyan-700/90" },
  { id: "b3", title: "Vibe Coding 时代 — 非程序员的顶级工具推荐", cta: "查看推荐", link: "#", active: true, gradient: "from-amber-600/90 via-orange-600/80 to-rose-700/90" },
];
...
export function HomeBanner({ slides: propSlides }: HomeBannerProps) {
  const { data: dbSlides } = useBannerSlides();

  // Priority: props > DB > fallback
  const allSlides = propSlides || dbSlides || defaultBannerSlides;
            ...
            <CarouselItem key={slide.id}>
              <div className={`relative overflow-hidden rounded-2xl bg-gradient-to-br ${slide.gradient} aspect-[3/1] flex items-center`}>
                ...
                <div className="relative z-10 px-8 md:px-12 lg:px-16 w-full">
                  <h2 className="text-xl md:text-2xl lg:text-3xl font-bold text-white leading-tight max-w-2xl drop-shadow-lg">{slide.title}</h2>
                  <Button size="sm" className="mt-4 bg-white/20 hover:bg-white/30 text-white border border-white/30 backdrop-blur-sm gap-1.5">
